...
/**
 * This class is an UI that allows triggering an action by pressing a button. In
 * particular, it is also possible to choose an element from a list before
 * triggering the action (this is usually used to create a new row, where
 * sometimes you need to add an initial information to the row).
 * <ul>
 * <li>UI with button: select {@link #mode} to
 * {@link RowCreationMode#STANDARD}</li>
 * <li>UI with catalogue list and button: select {@link #mode} to
 * {@link RowCreationMode#SELECTOR}</li>
 * </ul>
 * Note that when a catalogue value is selected
 * {@link CatalogChangedListener#catalogChanged(Selection)} is called. Moreover,
 * if the button is pressed
 * {@link CatalogChangedListener#catalogConfirmed(Selection)} is called. To
 * select a catalogue call {@link #setList(String)}. Note that also a label is
 * shown on the left of the catalogue selector/button if
 * {@link #setLabelText(String)} is called.
 * 
 * @author avonva
 * @author shahaal
 *
...
public class RowCreatorViewer {
	...
	/**
	 * Create object with tse list and button to confirm a selection note that in
	 * order to make it visible, you need to set a label or a list with
	 * {@link #setLabelText(String)} or {@link #setList(String)}
	 * 
	 * @param parent
	 */
	...
	/**
	 * Set an xml list for the combo box. All the values in the list will be picked
	 * up. If a filter needs to be set, please see {@link #setList(String, String)}.
	 * 
	...
	/**
	 * Set an xml list for the combo box and get only a subset identified by the
	 * selectionId. The selection id identifies a sub node of the xml list and
	 * allows taking just the values under the matched node.
	 * 
	 * @param selectionListCode
	 * @param selectionId
	 */
	...
	/**
	 * Listener which is called when the selection changes or when the button is
	 * pressed.
	 *
	 */
