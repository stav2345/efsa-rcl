...
/**
 * Class to manage and solve a
 * RELATION{parentTableName,parentColumnId.code/label} statement
 * 
 * @author avonva
 *
 */
public class RelationFormula implements IFormula {
	...
	public void compile() throws FormulaException {
		...
		if (split.length != 2) {
			throw new FormulaException(
					"Wrong RELATION statement, need .code or .label, found " + field + " in " + formula);
		}
	...
	public String solve(TableRow row) throws FormulaException {
		...
		if (colVal == null) {
			...
			return "";
			// throw new FormulaException("Formula: " + formula + ": No parent data found
			// for " + r + " in the row " + row);
		}
		...
		if (parentValue == null) {
			throw new FormulaException("No parent data value found for " + parentColumnId + " in the row " + row
					+ " with parent " + parent);
		}
